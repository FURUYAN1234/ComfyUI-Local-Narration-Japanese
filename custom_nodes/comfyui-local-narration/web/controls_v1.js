...
const labels={character:"Character / 声のキャラクター",review_readings:"Reading review / 生成前に読み確認",text:"Script / 読み上げ原稿",purpose:"Purpose / 用途・おまかせ指示",mode:"Control / 設定方法",engine:"Model / 音声モデル",voice_mode:"Voice source / 声の作り方",speaker:"Speaker / 用意された話者",style:"Voice and tone / 声質・口調",speed:"Speed / 話速（0＝おまかせ）",reference_text:"Reference transcript / 参照音声の原稿",gemini_voice:"Gemini studio voice / 公式ボイス",gemini_voice_design:"Gemini custom voice / 声デザイン",gemini_voice_id:"Gemini saved Voice ID / 保存済みID",gemini_emotion:"Gemini emotion / 感情",gemini_emotion_strength:"Gemini emotion strength / 感情の強さ",gemini_emotion_custom:"Gemini custom emotion / 感情の自由入力",seed:"Seed / 候補番号",pitch_semitones:"Pitch / 音程（半音）",volume_db:"Volume / 音量（dB）",pause_ms:"Sentence silence / 全体音声の台詞間の無音（ms）",irodori_steps:"Irodori steps / 生成ステップ",irodori_text_strength:"Irodori text / 原稿への強さ",irodori_style_strength:"Irodori style / 口調への強さ",irodori_voice_strength:"Irodori reference / 参照声への強さ",irodori_duration_scale:"Irodori duration / 予測尺の倍率",qwen_temperature:"Qwen temperature / 変化の大きさ",qwen_top_p:"Qwen top-p / 候補の累積確率",qwen_top_k:"Qwen top-k / 候補の数",qwen_repetition_penalty:"Qwen repetition / 反復抑制"};
...
app.registerExtension({name:"LocalNarration.Controls",nodeCreated(node){
 ...
 node.addWidget('button','Download models / 必須モデル一式を取得',null,async()=>{try{const r=await api.fetchApi('/local-narration/download-models',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({node:node.id,client_id:api.clientId})});const d=await r.json();if(!r.ok)throw Error(d.error);alert('モデル一式の準備ができました。');}catch(e){alert(e.message);}});
 }
