...
for mode in ['手動','AIおまかせ','AI提案＋手動上書き']:
 args.update(mode=mode,dialogue_blocks=json.dumps({'blocks':[{'id':'a','text':'一文目です。'},{'id':'b','text':'二文目です。'}]}))
 p,_=ctx['NarrationDirection']().plan(**args)
 assert p['text']=='一文目です。\n二文目です。' and len(p['dialogue_blocks'])==2
args['speed']=0.1
